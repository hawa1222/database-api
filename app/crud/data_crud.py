...
async def create_database(
    ...
    try:
        ...
        if 'database exists' in str(e).lower() or e.orig.args[0] == 1007:
            error_message = (
                f'Database "{database.db_name}" already exists: {str(e.orig)}'
            )
        ...
        # Catch all other errors and log error message
            ...
            error_message = (
                f'Error creating database "{database.db_name}": {str(e)}'
            )
...
async def create_db_user(db: AsyncSession, user: auth_schemas.DBUserCreate):
    ...
    try:
        ...
        error_message = (f'Error creating or updating DB user '
                         f'"{user.username}": {str(e.orig)}')
        logger.error(error_message)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=error_message
...
async def set_user_privileges(
    ...
    await db.execute(
        text(
            f'GRANT {user.privileges} ON {user.db_name}.* TO :username@:host'
        ), {'username': user.username, 'host': user.host}
    )  # Construct & execute query: grant privileges
    await db.execute(text('FLUSH PRIVILEGES'))  # Flush privileges
    await db.commit()  # Commit transaction

    logger.info(f'DB user privilges set to "{user.privileges}" '
                f'on "{user.db_name}"')
...
async def create_table(db: AsyncSession, table_info: data_schemas.TableCreate):
    ...
    try:
        ...
        message = (f'Table "{table_info.table_name}" created successfully '
        ...
        if 'already exists' in str(e).lower() or e.orig.args[0] == 1050:
            ...
            # Catch all other errors and log error message
            error_message = (f'Error creating table "{table_info.table_name}"'
                             f': {str(e.orig)}')
            logger.error(error_message)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def generate_create_table_query(db_tb_name, fields):
    ...
    field_definitions = ', '.join([f"`{name}` {type}" for name,
                                   type in fields.items()])
...
async def get_table(db: AsyncSession, table_fetch: data_schemas.TableIdentify):
    ...
    try:
        ...
        message = (f'Fetched table "{table_name}" '
                   f'from database "{db_name}"')
        logger.info(message)  # Log success message

        return {table_name: data_schemas.TableData(
        ...
        if 'doesn\'t exist' in str(e).lower():
            ...
            # Catch all other errors and log error message
            error_message = (f'Error fetching table "{table_name}"'
                             f': {str(e)}')
            logger.error(error_message)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def insert_data(db: AsyncSession, data_insert: data_schemas.TableData):
    ...
    try:
        ...
        for row in data_insert.data:
            ...
            data_dict = {key: (value if value is not None else None)
        ...
        message = (f'Datinsertion completed for table '
        ...
        if 'doesn\'t exist' in str(e).lower() or e.orig.args[0] == 1146:
            error_message = (f'Table "{data_insert.table_name}" does not '
...
async def delete_table(
        db: AsyncSession,
        table_delete: data_schemas.TableIdentify
):
    '''
    ...
    try:
        ...
        await db.execute(query)  # Execute query
        await db.commit()  # Commit transaction
        message = (f'Table "{table_delete.db_name}" deleted successfully '
        ...
        if 'unknown table' in str(e).lower() or e.orig.args[0] in [1146, 1051]:
            error_message = (f'Table "{table_delete.db_name}" does not exist '
