...
async def non_admin_access_token(access_token):
    ...
    async with AsyncClient(
        ...
        payload = {"username": Settings.TEST_USER, "password": Settings.TEST_PASSWORD}
        response = await client.post("/get-token", data=payload)
        headers = {"Authorization": f"Bearer {response.json()['access_token']}"}
...
async def test_get_token_pw_fail():
    async with AsyncClient(
        ...
        payload = {"username": Settings.API_ADM_USER, "password": Settings.TEST_DB_NAME}
        response = await client.post("/get-token", data=payload)
...
async def test_non_auth_register_api_user(non_admin_access_token):
    ...
    async with AsyncClient(
        ...
        logger.info(
            f"!!!!!!!! Response test_non_auth_register_api_user: {response.json()}"
        )
...
async def test_create_user_already_exists(access_token):
    ...
    async with AsyncClient(
        ...
        logger.info(
            f"!!!!!!!! Response test_create_user_already_exists: {response.json()}"
        )
...
async def test_create_user_invalid_privileges(non_admin_access_token):
    ...
    async with AsyncClient(
        ...
        assert "Unauthorised access" in response.json()["detail"]
        logger.info(
            f"!!!!!!!! Response test_create_user_invalid_privileges: {response.json()}"
        )
...
async def test_create_table_already_exists(access_token):
    ...
    async with AsyncClient(
        ...
        assert "already exists" in response.json()["detail"]
        logger.info(
            f"!!!!!!!! Response test_create_table_already_exists: {response.json()}"
        )
...
async def test_create_table_invalid_schema(access_token):
    ...
    async with AsyncClient(
        ...
        payload = {
            ...
            "table_schema": {"id": "INVALID_TYPE", "name": "VARCHAR(50)", "age": "INT"},
        }
        response = await client.post("/create-table", json=payload, headers=headers)
        assert response.status_code == 500
        assert "Error creating table" in response.json()["detail"]
        logger.info(
            f"!!!!!!!! Response test_create_table_invalid_schema: {response.json()}"
        )
...
async def test_insert_data_table_not_exists(access_token):
    ...
    async with AsyncClient(
        ...
        logger.info(
            f"!!!!!!!! Response insert-dat(table not exists): {response.json()}"
        )
...
async def test_delete_table_not_exists(access_token):
    ...
    async with AsyncClient(
        ...
        assert "does not exist" in response.json()["detail"]
        logger.info(
            f"!!!!!!!! Response test_delete_table_not_exists: {response.json()}"
        )
